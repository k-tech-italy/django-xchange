...
@pytest.mark.parametrize(
    'initial, refresh, include, requested, expected', [
        pytest.param(None, False, None, 'EUR,GBP,USD',
                    {'EUR': 1.0, 'GBP': 0.839441, 'USD': 1.084928}, id='empty'),
        pytest.param({'ABC': 0.9, 'GBP': 1.1}, False, None, 'EUR,USD',
                     {'ABC': 0.9, 'EUR': 1.0, 'GBP': 1.1, 'USD': 1.084928}, id='existing'),
        pytest.param({'EUR': 0.9, 'GBP': 1.1}, False, ['GBP'], 'EUR,USD',
                     {'EUR': 0.9, 'GBP': 1.1, 'USD': 1.084928}, id='not-overwrite'),
        pytest.param({'GBP': 1.1}, True, None, 'EUR,GBP,USD',
                     {'EUR': 1.0, 'GBP': 0.839441, 'USD': 1.084928}, id='force'),
    ]
)
...
def test_rate_get_rates(ensured, force, include, expected, monkeypatch, mock_rate_provider):
    # monkeypatch.setattr('django_xchange.models.currencies.Rate.ensure_rates',
